...
<head>
    ...
    <title>Личный кабинет <?= htmlspecialchars($_SESSION['user_email'] ?? '') ?></title>
...
<body class="page">
    ...
    <main class="main">
        <div class="container container--main">
            <h1 class="main__title">Ваш Личный кабинет,
                <?= htmlspecialchars($_SESSION['user_first_name'] ?? $_SESSION['user_email']) ?>
            </h1>
            <section class="features-section">
                <header class="features-section__header">
                    <h2 class="features-section__section-title">
                    <p>Ваше имя: <?= htmlspecialchars($_SESSION['user_first_name'] ?? "Имя не задано") ?></p>
                    <p>Ваша фамилия: <?= htmlspecialchars($_SESSION['user_last_name'] ?? "Фамилия не задана") ?></p>
                    </h2>
                </header>
                <form action="php/LC-update.php" method="POST" class="form">
                    <label for="email-input" class="form__input-label">Имя</label>
                    <input type="text" name="first_name" id="email-input" class="form__text-input" placeholder="Введите имя"
                        required>
                    <label for="password-input" class="form__input-label">Фамилия</label>
                    <input type="text" name="last_name" id="password-input" class="form__text-input"
            ...
            <section class="book-section">
                <header class="book-section__header">
                    <h2 class="book-section__title">Наличие подписки <?= htmlspecialchars($_SESSION['user_subscription_status']) ?></h2>
                </header>
            </section>
        </div>
    </main>
